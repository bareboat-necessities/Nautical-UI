#pragma once

#include "app/AppModule.h"

#include <gtkmm.h>

namespace helm::modules::nav {

class NavCanvas final : public Gtk::DrawingArea {
public:
    NavCanvas();
    void set_snapshot(const telemetry::TelemetrySnapshot& snapshot);

private:
    void on_draw(const Cairo::RefPtr<Cairo::Context>& cr, int width, int height);
    telemetry::TelemetrySnapshot snapshot_{};
};

class NavModule final : public app::AppModule {
public:
    NavModule();

    std::string id() const override { return "nav"; }
    std::string title() const override { return "Nav"; }
    Gtk::Widget& widget() override { return root_; }
    void on_telemetry(const telemetry::TelemetrySnapshot& snapshot) override;

private:
    Gtk::Box root_;
    Gtk::Label title_;
    Gtk::Label subtitle_;
    NavCanvas canvas_;
};

} // namespace helm::modules::nav
